...
The system consists of ten major functional blocks split across software (user-space C on Linux) and hardware (SystemVerilog on the FPGA fabric), connected through a Linux kernel driver and the Avalon memory-mapped bus.
...
\item \textbf{Rasterizer $\rightarrow$ Z-buffer / Framebuffer}: Direct BRAM ports at one access per pixel clock.
...
The edge function $E(x,y) = Ax + By + C$ is non-negative on the interior side for CCW winding.
...
Bit 1 & FFL & Command FIFO full; writes will be dropped. \\
...
\item \textbf{Linux device driver.} Implement \texttt{voxel\_gpu} platform driver bound to \texttt{csee4840,voxel\_gpu-1.0}. Expose \texttt{/dev/voxel\_gpu} with \texttt{write()}, the five ioctls from \S\ref{sec:quaddesc}, and \texttt{VGPU\_SET\_PALETTE}. User-space test program uploads a palette and streams pixels.
